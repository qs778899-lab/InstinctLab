...
>    - **观察空间 (Observations)**：无历史帧输入（只看当前帧），且包含了 `base_lin_vel` (基座线速度)。
> 
...
>    - **观察空间 (Observations)**：
>      - 引入了**历史帧（History）**输入（通常为过去 8-10 帧），使策略能隐式推断加速度、接触状态等动态特性。
>      - 去除了 `base_lin_vel`，加入了**重力投影（Projected Gravity）**来感知身体倾斜。
> 
> 
